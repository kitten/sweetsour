...
let parser (s: Lexer.lexerStream) => {
  ...
  let parseValueOrInterpolation (t: Lexer.tokenValue) => {
    switch t {
      ...
      | _ => {
        raise (LazyStream.Error "Unexpected token while parsing a value, expected a Word or Interpolation")
      }
    }
  };
  ...
  /* recognises all tokens that are valid as part of a declaration */
  /* NOTE UNUSED:
  let isDeclarationToken (t: Lexer.tokenValue) => {
    switch t {
      | Interpolation _
      | Paren _
      | Word _
  ...
  */
  ...
  let propertyLoop (): node => {
    ...
    let node = switch (parseToken (BufferStream.next buffer)) {
      | Some (Word str) => Property str
      | Some (Interpolation x) => PropertyInterpolation x
      | _ => {
        raise (LazyStream.Error "Unexpected token while parsing a property, expected a Word or Interpolation")
      }
    };
    ...
    switch (parseToken (BufferStream.next buffer)) {
      | Some Colon => ()
      | _ => {
        raise (LazyStream.Error "Unexpected token after parsing a property, expected a Colon")
      }
    };
  ...
  let compoundValueLoop (): node => {
    switch (parseToken (BufferStream.next buffer)) {
      ...
      | _ => {
        raise (LazyStream.Error "Unexpected token while parsing a compound value, expected a Word or Interpolation")
      }
    }
  };
  ...
  let rec parseDeclOrSelector (): node => {
    ...
    switch (parseToken token) {
      ...
      | None => {
        raise (LazyStream.Error "Unexpected EOF, expected selector or declaration")
      }
    }
  };
